...
2. **已答行**：键 ＝ 编号＋答复首个选项字（`Q-02a`／`Q-19①`；`(a)`／`A：`／`是` 也认，统一小写）。查表前先按此键查一次，未命中再剥掉编号末尾单个选项字符查一次（`Q-38b`→`Q-38`；`Q-380` 数字结尾不剥，因为它不是选项字母）——选项字母不参与键匹配，只记录答了哪个选项。有映射 ⇒ 生成映射任务（id `answer:Q-xx`，字段齐全、来源＝队列行号、同 id 不重复生成、状态由台账说了算），所列条目解阻塞（`待开／无`）；`前置于` 的条目改为依赖新任务；`保持阻塞` 逐 id 改类型并把原因写进产出判据前缀 `【Q-xx 已答 a·仍阻塞：…】`，且**跨行粘性**——任一已答行要求保持阻塞，别的已答行解不开它。
...
| `Q-27` | `.51` VC++ 2015–2022 x64 Redistributable 已由 `0917BB` 装完（2026-09-18 07:2x），登记为已完成 | — | `relay:R-9` 解阻塞；不生成新任务，`.51` 系统组件变更仍须人派专门 opener |
| `Q-38` | 继续搁置维持 `Q-07`『以后再补』，仅消除缺映射提示（2026-09-18 08:1x；答复文本首字符键为 `Q-38b`，剥选项字母归一化为 `Q-38` 后命中本行） | — | `answer:Q-07` 保持阻塞·外部（维持『以后再补』，⛔ 不再追问） |
