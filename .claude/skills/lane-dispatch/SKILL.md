...
- ⛔ **改 `docs/openers/run-lanes.sh` 的 opener 永远不进泳道**（2026-09-08 第十二批实证：脚本运行中被自己派的 opener 改写，
  bash 按旧字节偏移续读新文件，`run_lane` 被切成 `un_lane`，六条被派两遍）。这类改动单独出一份 CC opener、在发车前跑完；
  `0908U` 起脚本会自拷贝到临时目录再 exec，但"不进泳道"这条仍然保留——护栏是双份的
...
🔴 **同结果的方案只给一种**（人为参与最少、无歧义）。看护者报告的「下一步」同样：一条推荐动作 + 理由，
⛔ 不列 A/B 让他选；只有结果不同且属不可代项／真取舍才列选项并标推荐（2026-09-04 定，反例＝0903Y 报告的 A/B、C/D、E/F）。
