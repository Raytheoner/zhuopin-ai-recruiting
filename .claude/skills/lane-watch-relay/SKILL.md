...
| 执行仓库内 venv／二进制判「环境坏没坏」 | ✅ 有效 | ⛔ **假阴**——`tools/liaison/.venv/bin/python` 等是指向 macOS 解释器的符号链接，挂载侧执行必报 `No such file or directory`，看着像「venv 损坏」其实好的（`0921B` 实测：同一路径 Mac 侧机器判据 PASS、挂载侧仍报缺失） | 环境类判据一律放进泳道的「## 机器判据」在本机侧跑；挂载侧只能看文件在不在，⛔ 不得执行它、更不得据执行失败断言损坏 |
| Claude 浏览器面板抓网络请求判客户端故障 | ✅ 他自己的窗口才算数 | ⛔ **假阳**——面板对私网站点逐次授权，页面 XHR 会报 `net::ERR_BLOCKED_BY_CLIENT`（`0920R` 实测：面板里 4 个 `/api/*` 全 blocked，他本人窗口同一页 24 条正常） | 面板只用来判「页面能不能打开」（＝在不在网）；判客户端故障一律请他在自己窗口点一次并给图，⛔ 不得拿面板的 blocked 当现网证据 |
...
| `CTX-RELAY`（某条 opener 越 150k 被注入转场哨兵） | **＝正常**，`run-lanes.sh` 已自动在本泳道下一条（续棒条 `<原id>续<n>` 插在原条之后、其它条之前）排了续棒；⛔ 不当失败处理、不用去 `tail` 日志找错误 |
