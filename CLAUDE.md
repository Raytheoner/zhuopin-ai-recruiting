...
```
[Mac]MMDDX-<主题短名>
【设置】执行环境: CC ｜ Session: 新开 ｜ 分支: main ｜ worktree: ❌ 不勾（<理由>）｜ 工作区: 仓库根 ｜ 派发: <母 session 编号>
```

- **两行都在代码块内部**，不在外面——他复制整块时 CC 那边要能看到自己是谁、跑在哪
- **标题行是给复制进去的那个 session 看的**（`[Mac]` 标执行机、`-` 是 id 与主题的唯一分隔符）。⛔ 不写 `OP-` 前缀、不写 `CC ·`、不用空格分隔
- 🔴 **CC 的 opener 第 3 行必须调 `set_session_title`**（标题行**不会**自动变成 session 名，不调就丢编号）。⛔ Cowork 侧首行无效，不要改首行格式硬试。照抄句、豁免与机器判据见 skill `kickoff`「两条抬头规则的完整展开」
...
- **MMDD 必须实跑 `TZ=Asia/Shanghai date +%m%d` 取**：本机在 EDT，比中国晚 12 小时，照本机日期编会集体差一天且不报错。⚠️ **序号出前先查号池台账（`docs/openers/号池台账.md`，2026-09-16 起从编排文件拆出；历史月份在 `docs/openers/归档/`，`grep -r` 照样命中），给出后当场登记**——只在聊天里派、不落档的号下轮必被重派，08-27 一天撞了 4 次；Cowork 接力号与 CC 共用同一号池
- **worktree 后面的括号写理由**，六项一个都不能少、**用 ｜ 分隔写成一行**
- 🔴 **`派发` ＝ 出这份 opener 的母 session 编号**（2026-09-08 定），⛔ 不留空、不写"未知"——否则出问题回溯不到是谁派的。四种取值与机器判据见 skill `kickoff` 同一节
...
🔴 **凡能写成脚本的人工步骤，一律写成脚本**（2026-09-09 Shao Peishen 定：「以后能改成全自动都全自动，减少人工干预少出错」）。⛔ 不许因为「让他手动做一下更简单」就把人放进环里——人工步骤既占他时间又是出错源。自动化必须**自愈**（`trap` 恢复 ＋ 独立兜底）并带**无害预检**（权限不足当场退回人工并说明），⛔ 不许留下「脚本挂了就回不去」的状态。判据见 `.claude/skills/kickoff/SKILL.md`「能自动化的一律自动化」。
...
## 🧭 会话末「需你定夺」（2026-09-09 引入，源自企业AI转型 §5）

**每次回复末尾固定一个小节**，编号清单。⛔ 不许把待定事项散在正文里说「两件还在你手上」——
那种写法他读完不知道要答什么、答完谁动手，等于没交办。

1. **每项必须可直接作答**：带 `(a)/(b)/(c)` 或「是/否」标签，每个选项写清选它会发生什么与代价；清单末尾的答复模板**必须单独放一个无语言标签的 fenced 代码块**（内容只有 `1a，2a` 这一行，Desktop 会给它一键复制按钮；⛔ 不用行内反引号、⛔ 不标 `bash`——那会变成 Run 按钮），他改字母即答（2026-09-10 Shao Peishen 要求恢复）
2. **两栏各归各位**：状态另起「**状态同步（无需你答）**」。判据＝凡出现「等你定／请你选／待你拍板」即属「需你定夺」，串栏即违规
3. **路由只问一句：答完之后由谁动手？** 本 session 接着做 ⇒ 就地给选项＋默认项；要另起 session／对外发送／改他人触碰区 ⇒ **不给选项**，只写「已登记 <文件> #N，待派发」＋一句结论
4. **默认项只在两个前提下成立**：设前自问 ① 他不答，谁去执行？② 不答的代价是「停在原地」还是「错误继续发生」？答不出执行者、或是后者 ⇒ **不设默认项**，改标「本项无默认，须你明确答复」
5. **建议未答不得当默认通过**：牵动后续动作而未答的建议，下次交互重提。判据＝**机制只认状态、不认建议**——涉归属／优先级／范围／排期的，要么当场改状态字段，要么标「待他确认」，⛔ 不留中间态
6. 无决策项时明写「**本次无需你决策**」。临期项标日期置顶；此前悬置项以选择题形式重提

**滚动文件体积闸（2026-09-16 Token 治理 Phase 3，`0916D`）**：`session接力.md` ≤ 40 KB、`OP-0820-全量编排.md` ≤ 60 KB、`号池台账.md` ≤ 60 KB，由 `tests/test_doc_size_budget.py` 把关（不过即 pytest 失败、看护者不发车）。一件事闭环时，在它的 `##`/`###` 标题里加「【已闭环】」（或把 `###` 标题整个划掉 `### ~~…~~`），然后跑 `python3 scripts/archive_docs.py --apply`——原样搬进归档、带守恒校验，⛔ 不要手工剪贴、⛔ 不要为了过闸删内容。

**待办一律落进 `docs/session接力.md`，不留在聊天里**，每条带齐【谁做】【状态】【判据：怎样算完】【不做会怎样】四列——只在聊天里说的待办，他转场后必被漏。

## CLAUDE.md 收录规则（防膨胀）

本文件是每会话自动加载的**约束层**，不是项目档案。规则语料只增不减会稀释所有既有规则的
注意力份额。改本文件前必须过三关，**三条量化红线任一触发即拆**（拆＝超出部分挪进编号文档，
正文留结论 + 一行指针）：① 全文 > **250 行** ② 任一 `##` 小节 > **40 行** ③ 任一条规则正文 > **5 行**。

新增内容前自问一句：**"没有这条，下一个会话会立刻做错什么？"** 答不出具体错误就不进正文。

四类准入、两类禁入的完整清单与去处对照表见 `docs/CLAUDE-md-收录规则.md`。
