...
- **标题行带编号**，`OP-<月日>-<序>`。孤立重发单个代码块时，一眼能看出这是哪件任务
- **worktree 后面的括号写理由**（如"只出分析与方案，不写代码"），不要只写符号
- 五项一个都不能少，**用 ｜ 分隔写成一行**，不要拆成四行
- **CC 与 Cowork 两端同等适用。**「这只是中途一条命令／让他执行某个文件，不算 Opener」**不是豁免**——判据是"这段会不会被复制到另一个界面去"，会就必须带。侧边栏只能拿首句猜会话名，编号一断，跨会话就查不回这是哪件任务（实证：2026-08-26 近期会话全部丢号）
- CC 侧有 `Stop` hook 机器校验这条，见 `scripts/hooks/check-opener-header.py`
