...
`~/Library/CloudStorage/OneDrive-Personal/Projects/企业AI转型/`

- ✅ **可以完全读取**全部内容
- ❌ **不得修改、不得删除、不得 lock**（不要用会持有文件锁的方式打开）

跨端**写入**会造成文件锁冲突与覆盖（该目录由另一台 Windows 机器通过 OneDrive 维护）；**读取**没有这个问题。
...
**遇到"这个要不要自己造"，先去那边看有没有现成的。** 引入方式是**读取参考 + 在本仓库自建实现**，不要跨仓库引用或拷贝文件形成耦合——两个仓库独立演进，共享的是做法不是代码。
