...
def _repeats_earlier_assistant_turn(candidate_text: str, history: list[dict]) -> bool:
    ...
    **tasks 5.8 的结论（2026-08-26，交付单元 E）：保留，职责收窄为兜底。**
    ...
    1. **台账为空的 job 上，本函数是唯一的防线。** `.51` 现网的既有 job
       （`delivery-units.md` §5 约定 4 记作 15 个）在第 8 章 8.3 升级到单元 B
       的新列之后，`asked_questions` 全是列默认值 `'[]'`——加列时按约定不回填
       历史行（同条约定；本机 demo 库的加列演练实测 22 行全部拿到默认台账，见
       `docs/findings/2026-08-26-unitB-已问台账列加列演练.md` 结论 3）。这些
       会话继续对话时台账恒为空，一个重问标记都不会打、重问上限一次都不会触发。
       删掉本函数，它们当场退回 2026-08-11 之前的行为。
