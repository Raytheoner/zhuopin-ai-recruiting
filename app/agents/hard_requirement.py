...
# ⚠️ "意识"命中的代价不止一条技能行：_extract_functional_safety 后来接了
# 同一道过滤，"功能安全意识"写进 functional_safety 字段会让整条 ASIL 门槛
# 都不产出（不是丢一行 core_skills，是丢一整个字段的门槛）。仍是同一个
# 接受的方向（宁可少一条规则），但影响半径比写这段注释时更大，一并记下。
#
