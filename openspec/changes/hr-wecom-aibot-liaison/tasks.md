> **进度**：64/67（第 1-7 章已完成并合回 main，第 8 章 7/10——2026-09-17 `0917V` 实证核验 8.6/8.7：8.7 勾（配置＋重启＋告知信均有真实证据），8.6 不勾（3 条验收面只 1/3 满足，详见 `docs/findings/2026-09-17-8.6与8.7实证核验.md`），8.8 开一周观察窗（09-17→09-24，`docs/findings/2026-09-17-值守通道一周观察窗.md`）。第 8 章 6/10 时的旧状态如下：⚠️ **2026-09-10 追加 8.5bis「SDK `message` 事件接线」**，总数 66→67：第 4 章「消息归档」与第 5 章「值守任务队列」虽各自 6/6、10/10，但**运行期入口从未接上**——`SUBSCRIBED_EVENTS` 只订三个连接事件，`test_this_chapter_wires_no_message_handling` 还明令不接 ⇒ 两章只在单测层面成立，现网 `liaison_message` 恒为 0 行。这是比 TD-42 更靠前的前置，立包时未登记。第 1/2/3 章 2026-09-08，第 4 章「消息归档」、第 5 章「值守任务队列」、第 6 章「群通知外发」与第 7 章「连接生命周期与中断告警」2026-09-09。各 6/6、6/6、6/6、10/10、10/10、10/10、9/9。第 8 章已勾 8.1「留存期清理」、8.2「清理失败告警」（2026-09-09 [Mac]0909K）、8.3「launchd 守护配置与安装脚本」、8.4「日志接入：轮转+有界+脱敏」、8.5「结构性守护测试」2026-09-09——⚠️ 8.3/8.5 都只到「配置与测试就位」，launchd **尚未实际安装**，装与灰度在 8.6。⚠️ 8.4 只做了**容量上界**，「留存期有上限」的时间维度**未实现**（已登记 TD-30，原计划由 8.1 用独立的 `HR_LIAISON_LOG_RETENTION_DAYS` 默认 30 收口，⛔ 不复用 180 那个变量——⚠️ **8.1 实际 ⛔ 未做这一条**，只做了归档与消息台账的 180 天，TD-30 仍未还，见第 8 章落地偏离登记 P3）。⚠️ 第 6 章同样只到「给了 URL 就能发」：真实群 webhook 尚未写进 `.env`，**未做过任何一次真实投递**，灰度真发同在 8.6 由 Shao Peishen 本人操作）
>
...
- [x] 8.5bis **SDK `message` 事件接线**（2026-09-10 追加；`liaison-reply-bridge-and-patrol` design D6 把接线判给本包，其 tasks §0.2 只做核验）。七件事：
  ① `session_client.py` 加 `EVENT_MESSAGE = "message"` 并入 `SUBSCRIBED_EVENTS`；
  ② **表面校验同步**：照 `EVENT_ERROR` 那段的形状，给 `message` 加一条「不在清单里就 `SdkSurfaceUnverifiedError` 拒绝启动」——⚠️ 漏订阅**不报错**，只是静默收不到，这正是 TD-39 能发生的成因之一（`session_client.py:300-307` 的注释已立此判据）；
  ③ 回调**只往队列放帧**（`msgid` / `sender_userid` / `msgtype` / `content` / 附件句柄 / 到达时刻），⛔ **不在 SDK 回调里碰 DB**——回调跑在事件循环线程，而铁律 1 要求幂等记录与业务写同一连接同一 `BEGIN`，那个连接归值守线程；跨线程用同一 sqlite 连接会把事务归属搅乱（`docs/findings/2026-08-13-sqlite-事务归属冲突.md` 同族）；
  ④ 值守线程取帧 → 调 `inbound.handle_inbound_message(conn, thread_id=…, msgid=…, sender_userid=…, received_at=…, msgtype=…, content=…, attachment=…)`；`thread_id` 按 design D3 取（私聊 userid／群聊 chatid）；
  ⑤ **帧字段→参数的映射按 SDK 实际帧取**，⛔ 不照文档猜。拿不到真实帧形状就按「表面未验即拒绝启动」处置（TD-19 同一处置），⛔ 不许先写个猜的映射让它静默跑错；
  ⑥ **删** `tools/liaison/tests/test_main_wiring.py::test_this_chapter_wires_no_message_handling`，并**换上正向断言**（`__main__.py` 确实 import `inbound` 且调到 `handle_inbound_message`、`SUBSCRIBED_EVENTS` 含 `message`）。⛔ 只删不换＝这块回到无判据状态，下一轮谁拆掉接线都不会变红；
  ⑦ 接线层幂等测试：同 `msgid` 重投 ⇒ 归档不出第二份、队列不增第二行、回复不重发。**幂等策略**：本条 ⛔ 不新增 `effect_*`——`handle_inbound_message` 内的 `archive_message` 已按 `{thread_id}:…:{msgid}` 幂等，本条只加接线层的恒等断言。
...
- [ ] 8.8 一周观察窗口的观察项落档：漏消息告警是否误报、限流是否被触发、归档是否有重名冲突。⚠️ 观察结论写 `docs/findings/`，⛔ 不在观察期内改判据
  > ⏳ **观察窗 2026-09-17 → 2026-09-24**（`0917V` 开窗，`docs/findings/2026-09-17-值守通道一周观察窗.md`）：三项各已记查法命令与当日基线值（漏消息告警命中 15 次累计／限流命中 0 次·结构上未接通前不可能触发／归档 `msgid` 主键约束下重名差值恒为 0）。到期后收结论再判是否可勾，⛔ 本行不勾
