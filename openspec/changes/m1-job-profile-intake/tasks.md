...
> ---
>
> **2026-09-08 复盘（上面这份 28 条是 2026-08-20 的快照，不再是现状）**：逐条勾选已推进到
> **60/72**，未勾只剩 **12 条**——**C 类 11 条已全部移出**（8 条企微 → `hr-wecom-aibot-liaison`
> 已立项；3 条调度 1.7 / 5.6 / 6.8 → 调度基础设施，待立项，2026-09-08 Shao Peishen 裁决），
> **D 类只剩 9.1**（10 个真实历史岗位重跑、HR 与业务经理双方评估——**要真人参与，不是代码能做完的**）。
> ⇒ **归档判定权在 Shao Peishen**：11 条 C 全在别的包里，唯一的 D 是一次人工验收。
> ⛔ 代理人不代拍——归档时机属"改变外部可观察行为"之外的账目动作，但 9.1 是否作为归档门槛
> 由本人定。上面 28 条的分组仍可查阅，作为当时的判断留痕保留。
>
...
- [ ] 1.7 写 checkpoint 清理任务（按流程完成时间归档），M1 不启用但代码就位
      ⤷ **已移出**（2026-09-08 Shao Peishen 裁决）→ 调度基础设施（待立项），见文末「已移出」清单。⚠️ 原登记去向是「M2 Postgres 迁移」，同日一并改判：清理任务的**载体是调度器**，不是 checkpointer 的表结构——即便 M2 迁完 Postgres，没有调度器它仍然跑不起来
...
- [ ] 5.6 业务经理超时：3 个工作日提醒一次，再 3 天置 `abandoned` 并保留已采集内容
      ...
      ⤷ **已移出**（2026-09-08 Shao Peishen 裁决）→ 调度基础设施（待立项），见文末「已移出」清单
...
> ⛔ 移出**不等于**不做——下面**仍待做的是 11 条**（8 条企微 + 3 条调度），都是"确实还要做"，
> 只是不在本包的归档门槛里。原登记写 12 条，账目变动两笔：**减 3**——1.3 / 2.6 随
> `ai-audit-trail-and-outbound-gate` 归档交付，2.3 划回本包由 `0908D` 交付，三条均已回勾；
> **加 2**——5.6 / 6.8 于 2026-09-08 新移入（1.7 原就在册，本次只改去向，不重复计）。
> 12 − 3 + 2 = **11**。逐条状态见下列各小节。
...
### → `hr-wecom-aibot-liaison`（2026-09-08 已立项，`openspec/changes/hr-wecom-aibot-liaison/`）
...
| 原条目 | 内容 | 依据 |
|---|---|---|
| 1.7 | checkpoint 清理任务（按流程完成时间归档） | 清理是**按时触发的后台动作**，载体是调度器。原登记去向「M2 Postgres 迁移」同日改判：迁完 Postgres 也仍然没有谁来按点跑它 |
| 5.6 | 业务经理超时：3 个工作日提醒一次，再 3 天置 `abandoned` | 「3 个工作日后提醒」这句话本身就是一条调度规则。没有调度器时 `JobStatus.ABANDONED` 只能停在枚举值上，无写入路径 |
| 6.8 | 挂起提醒：第 1 天、第 3 天各一次 | 与 5.6 同源。发提醒是有副作用的动作，必须落在 `effect_*` 节点里由真正的调度器触发 |

**依据**（2026-09-08 Shao Peishen 裁决）：M1 **没有 cron / 后台任务载体**——部署形态是
Windows 计划任务拉起 uvicorn 单进程（见 `04-部署与门户挂载.md`），Web 通道是同步
请求/响应，进程里没有任何按时触发的东西。这三条硬做等于**在本包里自造一个调度器**：
⛔ sleep 循环与后台线程不算数，进程一重启就没了，而这三条要的恰恰是"跨重启不丢"。
与 `docs/superpowers/plans/2026-09-04-m1-job-profile-intake-unit6-approval-checkpoint.md`
对 6.8 的留步、`docs/tech-debt.md`
**TD-11** 同源——那两处早已把这件事记成"等定时基础设施"，本次只是把账目从"本包欠着"
改成"不属本包"。

⚠️ 立项时要一并带走的口径：6.8 的触发点（第 1 天、第 3 天各一次）已写在本包 spec 的
「流程长时间挂起」Scenario 里；5.6 的 `abandoned` 置位是**有副作用的写**，落地时须独占
一个 `effect_*` 节点并带幂等键（工程铁律 1、2）。

### → M2 Postgres 迁移（尚无变更包，待立项）

| 原条目 | 内容 | 依据 |
|---|---|---|
| ~~1.7~~ ⚰️ **已改判去向**（2026-09-08 Shao Peishen 裁决，移到上面的「调度基础设施」） | checkpoint 清理任务（按流程完成时间归档） | ~~清理任务必须贴着 checkpointer 的具体表结构写。现在按 SqliteSaver 的 schema 写一份，M2 迁到 Postgres checkpointer 时整份作废重写——先做就是先扔~~ ⇒ 该理由只说明了**不该现在写**，没说明该由谁承接。真正的阻塞是没有调度器：迁完 Postgres 也仍然没有谁按点跑它。M1 数据量（`.51` 现网 15 个 job）离需要清理还很远这条不变 |

⚠️ **本小节现已空转**——1.7 是它唯一一条，移走后 M2 Postgres 迁移在本文件里不再承接任何条目。保留小节是为了不在编号上留空洞。
