> **进度**：33/33（§0 三条门槛 ＋ §1 P0 回件桥＋第九态 ＋ §2 P1 信号与打标即开班 ＋ §3 P2 拆件章程正本全部完成 ＋ §4 P3 口径点台账全部完成并合回 main，2026-09-17 `[Mac]0917H`；§5.1–5.4 通过，5.3 判通过订正见 `[Mac]0917R`/`0917S`；§5.5 记录补齐，2026-09-17 `[Mac]0917U`，全部通过）。立包 2026-09-10（`[Mac]0909AT`）。需求树见 `intent.md`，裁决 Q1–Q7 见 `design.md` D1–D7。
> 全部通过，本包已具备归档条件，见下方归档记录。
...
- [x] 0.1 核验 **TD-42 已销账且有真实验证记录**：`docs/tech-debt.md` 里 `~~TD-42~~ ✅ 已还（<commit>）` 存在，且销账行引用了一次真实的「假死→自终止→launchd 拉起」实测（⛔ 单测不算）。未满足 ⇒ 本包停在此处，登记接力文档（2026-09-16 14:13–14:27 CST `[Mac]0910A` §四全自动断网脚本核验：9 分钟断网六项判读全过——判死/看门狗判定/终止退出码5/告警行/pid 由 41272 变 44355/复网自愈，`watchdog.json` `consecutive:1`）
- [x] 0.2 核验 **SDK `message` 事件接线已完成**（`hr-wecom-aibot-liaison` 追加任务 8.5bis，design D6）：`session_client.SUBSCRIBED_EVENTS` 含 `message`；`__main__.py` 值守线程调 `handle_inbound_message`；`test_this_chapter_wires_no_message_handling` 已删。未满足 ⇒ 停，登记待派发（2026-09-13 `[Mac]0913A` 核验：三条均在 main 上已满足，实现由 `[Mac]0910B AT-1a` 于 2026-09-10 完成并合回，commit `7636613`/`cd48966`；帧字段映射走 **fail-closed**）
- [x] 0.3 核验 **一条真实入站已落库**：`data/liaison.db` 的 `liaison_message` ≥1 行、来源为真实企微消息（⛔ 不是测试库）。这是 F1「接线缺失单独就足以造成 0 行」的反证（2026-09-16 `[Mac]0910A` AT-1b 两轮核验：`msgid=2a376681026302c7c9343bdf78aac10f`、`sender_userid=thread_id=ShaoPeiShen`（私信，`thread_id` 正确取 `userid`）、`received_at=2026-09-16T13:55:09+08:00`；`liaison_task` 同步 +1）
...
🔴 **落地偏离**：4.2 字面签名 `compute_criteria_transition(text, *, id, to, evidence)` 缺一个时刻值——「更新」列需要落一个日期，纯函数又不能内部读系统时钟。实现计划（`docs/superpowers/plans/2026-09-10-liaison-criteria-ledger.md`）已按此加了必填关键字参数 `today: datetime.date`，与 `followup.py::compute_backfilled_ledger` 同款处理，⛔ 不改 `design.md`，本行即偏离记录。

- [x] 4.1 建 `docs/跟进信/口径点台账.md`：表头 `口径点ID | 来源信 | 描述 | 状态 | evidence | 更新`；抬头写四态语义与「`已签认` 必须带 evidence、⛔ 无超期自动签认」；把人事部#1 的决策点 a 登记为 `HR-G-01`（状态 `待专员`，evidence 空）
...
- [x] 5.3 **P0+P1 端到端验**：等汤丽萍下一条真实入站（人事部#1 在途）——判据＝台账该行进第九态且原状态接后；`liaison_unpack_audit` 有 `bridge_marked` 与 `dispatch_started` 各一条；信号文件有一项；无头日志显示会话走完一轮并 `--clear --before` 清了信号；`git log` 有会话的 commit 且 ⛔ 未 push
...
- [x] 5.5 把 5.2–5.4 的实测记录（命令、时刻、审计表三条 SELECT 输出、日志前 20 行）落 `docs/findings/2026-MM-DD-打标即开班真实起活实测.md`；全部通过后 **当场**跑 `openspec-archive-change`（CLAUDE.md「归档时限」）（`[Mac]0917U` 2026-09-17：补录逐字命令/时刻/`sqlite3 -readonly` SELECT 输出/日志全文，见 findings 文件「七、5.2/5.3/5.4 补录」节）
