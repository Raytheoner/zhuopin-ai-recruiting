...
| **U7** 边界守护与文档 | 第 7 章（**7.3/7.4 已提前，见 §2.U7**） | 7.1/7.2 ＋ 7.5/7.6 | `.github/workflows/ci.yml`｜`docs/`（新增一页）｜`06-企业AI转型资产借鉴清单.md`｜`07-开发环境现状与优化待办.md` | 前面全部 | —— | 6 → **2-3**，见 §2.U7 |
...
2. **7.5 / 7.6 按清单执行** —— 两条技术债登记（`operator_id` 不可信、JSONL 仅进程内锁）。只改文档不改代码

> 🔴 **7.3 / 7.4 已于 2026-08-28 从本单元的尾部排期中提出**（Shao Peishen 裁决，落档见
> `docs/findings/2026-08-28-Shao-Peishen-五条裁决落档.md` §5）。
>
> **为什么**：`tasks.md` 7.3 写着「**U5 接线前必须确认本条已落地**，否则总开关在 `.51` 上不具备
> 可操作性」，而本文件原把整个第 7 章排在最后——**排期与前置条件互相打架**，按任一侧执行都会
> 违反另一侧。7.3/7.4 是纯文档，拆出来先做就解，不必等 U7 整章。
>
> **现状**：两条已于 2026-08-28 落地并回勾（`db6596e`），产出 `docs/audit-and-outbound-ops.md`。
> ⚠️ 该页第五节仍有三项 `.51` 上机留步，**第 3 项（在 `.51` 上按 4.1 实际创建一次开关文件）
> 仍是 U5 开工前的未闭合前置**，属不可代项。
>
> ⚠️ **编号提醒**：裁决原话说的是「7.1」，实际条目号是 **7.3**——7.1/7.2 是禁止
> `zhuopin_platform` 依赖那两条 CI 检查，与开关文件编码无关。按 7.3 落地。
...
### 3.5 `CANDIDATE_OUTBOUND_ENABLED` 的默认值 → ✅ **已拍板（2026-08-26 Shao Peishen）**

**三项结论，U4 可开工：**

| 项 | 结论 |
|---|---|
| **（一）代码默认值** | **A —— 代码默认关闭，`.env` 显式开启。** 与 tasks 4.5 现有口径一致，无需改常量。接受"上线初期什么都发不出去"的观察期，运维文档要写明去哪儿开 |
| **（二）谁有权开** | **只有 Shao Peishen 本人可开**；运维执行需拿到当次授权并留痕。按 CLAUDE.md 决策代理表，本项属不可代，代理人未指定期间不设例外 |
| **（三）配置形态** | **允许热改，不重启生效。** 落成每次外发时求值（可热改配置或每次读环境），**不得启动时缓存一次**。理由：出事时要能立刻全拦，而不是等重启——`.51` 上还跑着另外 7 个服务，为改一个开关重启整机的代价不成比例。这正是这道结构闸存在的意义 |

**对实现的两条硬约束**（抄进 U4 的 Global Constraints）：
1. `CANDIDATE_OUTBOUND_ENABLED` **必须每次外发时求值**。⛔ 禁止在模块导入期、`__init__`
   里、或任何单例上把它读成一个常量。tasks 4.5 已写"支持传 callable"，用那条路。
   守护测试：运行中改值后**不重启**，下一次外发要立刻按新值走。
2. **合并时保持关闭**（全拦），观察拦截留痕符合预期后再开，与 design 迁移计划第 4 步一致。

---

以下为拍板前列出的选项与后果，保留供追溯。
...
~~**在他拍板前**：U4 不开工（4.5、4.8 直接建在这上面）。~~
**✅ 2026-08-26 已拍板，U4 无阻塞可开工。** 结论取 A（默认关）+ 允许热改，见本节开头。
U1 按 tasks 4.5 的「关闭」写默认值即可，**不需要改常量**；但 U1 加配置键位时就要按
「每次求值、不缓存」的形态落，不要落成模块级常量。
...
② U2（第 2 章：app/audit）  ∥  U4（第 4 章：app/outbound 纯函数）
       ↓                              ↓
③ U3（第 3 章：留痕接线）    ∥   ★ 7.3/7.4（运维口径页 + 06 清单，纯文档）
   （U3 与 M1 全并行）                    ↓
       ↓                          U5（第 5 章：队列与图节点接线）
       ↓                          （U5 必须与 M1 的 B/D/E/F 串行，
       ↓                            建议插在 M1 的 F 合并之后）
       ↓                              ↓
④ U6（第 6 章：合规断言、对账与 CI）
       ↓
⑤ U7 剩余（第 7 章：7.1/7.2 走 plan，7.5/7.6 按清单执行）

★ = 2026-08-28 从 U7 提前到 U5 之前（Shao Peishen 裁决，见 §2.U7）。
    原排期把整个第 7 章放在最后，与 7.3「U5 接线前必须确认本条已落地」互相打架。
```
