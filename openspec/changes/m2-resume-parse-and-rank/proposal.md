...
- **模型境内、数据不出境**：沿用 M1 LLM 网关的供应商白名单；BGE-M3 部署位置在 design 决策中锁定为境内（本地或境内托管）。
...
**新增依赖**（W1 单元 U0 实测后定型，⛔ 不提前锁）
- 版面解析：PaddleOCR 或 MinerU（Windows 无 Docker 环境可装性是选型硬约束）
- Word 解析：`python-docx`
- 向量：BGE-M3（本地 CPU 推理或境内托管 API）；向量索引在现有 SQLite 上用进程内实现还是引入 pgvector，见 design Open Questions
