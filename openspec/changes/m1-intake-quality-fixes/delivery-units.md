...
> **决策状态：2026-08-19 Shao Peishen 已就三项全部拍板，本方案确认执行。** 三项均取本文档的推荐路：
> ① §3.2 Coverage Gap 走**窄化路（路 B）**——spec 已改，归档阻塞已解除；
> ② §3.1 F **排最后**（B∥C → D → E → F → G），接受编造率 20 场样本的时钟晚开始一轮；
> ③ §3.1 **D 先于 E**。
> 下一步＝B 与 C 各出一份 `spec-to-plan`。
...
**进这个单元时要先决策**：
1. ~~Coverage Gap「重试全部失败时记录累计耗时」走哪条路~~ → **✅ 2026-08-19 已定走窄化路（路 B），spec 已改，对 B 零影响**：3.9/3.10 不需要多加分支。详见 §3.2
2. 已问台账落哪儿：新列（走 1.1 已建立的 `init_schema` 幂等加列路径，决策 10）还是 `profile_json` 的下划线内部键（与 `_jd_text` 同位置，决策 8 的做法）。1.1 的加列清单里**没有**这一项，是新决定。**这一项属技术方案，按 CLAUDE.md 决策代理表可在 plan 阶段由 `spec-to-plan` 提方案、`run-build` 的 design review 定夺，不必回头找 Shao Peishen**
...
**两处不一致 —— ✅ 2026-08-19 Shao Peishen 已拍板，两处均取「按业务感受排」：**

- **F 排最后**，接受编造率的 20 场样本时钟晚开始一轮。执行顺序＝ B∥C → D → E → F → G（§6 不变）
- **D 先于 E**，与技术便利一致：D 合并后 E 的 5.5「重问超限 → 计入未指定字段」自动成立，不必写平行标记逻辑再拆

原始论证保留在下方，供日后复盘时看取舍点：
...
### 3.2 那条半实现的 SHALL → ✅ 已决（窄化路 B，2026-08-19），不再挂在 B 上
...
**✅ 已决 · 2026-08-19 · 决策人 Shao Peishen · 取路 B（窄化 spec）**

已落地，**不再是 B 的开工前置**：

- `specs/intake-turn-observability/spec.md`「逐轮时序留痕」原句已改写为只覆盖"重试后成功"，并新增「整轮失败不留痕」Scenario，把"该轮视为未发生"写成正面契约（含口径说明段）
- `design.md` Coverage Gaps 该条已销号，**归档阻塞解除**
- **对 B 的影响：零。** 3.9 的 `is_productive` 判定不需要多加分支，3.10 的预算口径不需要多加规则。B 开工时按现有 spec 直做即可
- **接受的代价**：整轮失败的耗时不可从持久层度量，只能从运行日志侧观察。日后若需要这个数字，另开变更走独立的失败计数表，**不得复用画像行**
...
**✅ 开工前的两个决定已于 2026-08-19 全部落地，B 与 C 无阻塞可开工**：
1. ~~Coverage Gap 走路 A 还是路 B（§3.2）~~ → **路 B，spec 已窄化**，对 B 零影响
2. ~~F 的排位（§3.1）~~ → **排最后**，上图顺序即最终顺序

**下一步**：B 与 C 各出一份 `spec-to-plan`（CC / 新开 session / main / 不勾 worktree）。两份可并行——B 全在后端、C 只碰 `index.html`，触碰区零重叠，前提是 C 守住 §5 约定 2「点选文本原样提交、不改 API 契约」。
