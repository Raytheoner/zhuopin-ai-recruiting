...
> **本章进度（2026-09-03 `0903L` 8.4 回勾后）：68/69**（本变更包全量 69 条，已勾 68）。剩 **8.9**（归档，须等 `m1-job-profile-intake` 先归档）。
>
...
- [x] 8.4 推送并重启后，用一个新 job 手工跑通"模糊回复 → 拿到档位 → 点选 → 带缺口确认"整条路径
  - **2026-09-03 完成**（`[Mac]0903L`）。依据：Shao Peishen 2026-09-03 14:05 在 Cowork
    会话 `HR业务线-接力0903B` 回「都要上」，本人在 `.51` 页面新建 job（`job_id`
    `51b225f1-1ede-49a1-8c6f-06b6ce401058`，采购员岗位）手工走完整条路径并回报
    「跑完了」附截图，session 随后从 `data/demo.db` 只读取证核对三条判据：
    ① `job_profile` v1–v5 的 `asked_questions` 均非空（追问真实发生，v6 收尾轮为空）；
    ② v3 的 `written_fields` 含 `soft_skill_keywords`、v4–v6 含 `core_skills`，
    对应页面点选的 chips（与截图逐一核对一致）；③ 末轮（v6）
    `derived_unspecified_fields` = `["autosar_experience", "functional_safety",
    "mcu_family", "diag_stack", "sop_projects", "toolchain"]`（6 项），与页面
    "带缺口确认"红框列表逐项对应。三条判据全部满足。
