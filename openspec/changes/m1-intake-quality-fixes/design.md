...
**当前无未决 Coverage Gap。** 唯一一条已于 2026-08-19 由 Shao Peishen 决策关闭，记录如下。

- ~~**`intake-turn-observability` 的「逐轮时序留痕」，"LLM 调用失败或重试时，留痕 SHALL 记录直到本轮结束为止的累计耗时" 这句 SHALL 的"全部重试都失败"这一半未实现**~~（unit A 收尾复核，2026-08-19 提出）。已实现并有测试覆盖的是"重试后成功"：耗时按重试次数累加，最终落进画像行。未实现的是"重试全部失败"：这种情况下 `SchemaExtractionFailed` 从 `run_intake_turn` 抛出，`effect_persist_draft` 不会跑，本轮既不写 `job_profile` 行也不写时序。
  - **✅ 已关闭 · 2026-08-19 · 决策人 Shao Peishen · 走窄化路（路 B）**：spec 原文已改，那句 SHALL 窄化为只覆盖"重试后成功"，并新增「整轮失败不留痕」Scenario 把"该轮视为未发生"写成正面契约。见 `specs/intake-turn-observability/spec.md`「逐轮时序留痕」及其口径说明。
  - **决策理由**：另一条路（整轮失败也写一行承载耗时）会让"系统炸了"的轮次伪装成"用户交互过的一轮"，而第 8 章的回放对比正是按 `job_profile` 行数算轮数——那会污染本批唯一的效果度量。同时它要给 3.9 的 `is_productive` 判定多加一个分支、给 3.10 的预算口径多加一条规则。
  - **接受的代价**：整轮失败的耗时不可从持久层度量，只能从运行日志侧观察。若日后需要这个数字，应另开变更走独立的失败计数表，而不是复用画像行。
  - **归档阻塞解除**：本条不再阻塞 `m1-intake-quality-fixes` 归档。
