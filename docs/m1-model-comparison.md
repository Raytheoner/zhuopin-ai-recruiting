...
> 状态：**已定稿（2026-08-11）**。单供应商实测完成（2026-08-09，DeepSeek，含
> deepseek-v4-flash vs deepseek-v4-pro 对比），**决策者已明确拍板：M1 demo 阶段
> 就用 DeepSeek，不等火山方舟（doubao）/ 阿里百炼（qwen）注册账号补测**——不是数据
> 缺失下的权宜之计，是显式决策。doubao / qwen 两行的模型名和 `json_schema` 支持情况
> 仍是候选阶段的占位猜测，未经实测；若之后要重新评估供应商（例如遇到 DeepSeek 侧的
> 延迟/质量/合规问题），再补真实 key 重跑本脚本。
...
- **本轮结论的性质**：单供应商验证 + 同供应商内 flash/pro 对比，不是"跨供应商实测对比后选优"——
  doubao/qwen 从未真实跑过。**2026-08-11 决策者显式拍板：M1 demo 阶段不等 doubao/qwen 补测，
  直接定 DeepSeek**，不是"数据不全下的临时选择"。
...
- **理由（选 DeepSeek 而非其他供应商）**：唯一持有 API key 的供应商；实测 schema 遵循度通过
  ...
  没有意义）。
- **待办**（非阻塞，不影响当前决策，供以后需要重新评估供应商时参考）：
  1. ~~注册火山方舟（doubao）和阿里百炼（qwen）账号拿到 key 后重跑对比~~——**2026-08-11 决策者
     明确不等这项**：M1 demo 阶段就用 DeepSeek，doubao/qwen 补测仅在以后有换供应商的实际动机
     （如遇到延迟/质量/合规问题）时才重新拾起，不再是本轮的待办阻塞项。
