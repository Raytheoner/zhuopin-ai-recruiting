...
**普通断网复现不出来**。N-0 若要端到端覆盖，另立一条。

**⚠️ 顺带发现（`0909AJ` 只登记、⛔ 未修，归另一条 opener）· 启动瞬间看门狗误触发**：
进程刚起、连接尚未建立时，看门狗读到**上一次运行残留的**存活戳（本次是 55 分钟前），
当场判陈旧并刷两条 ERROR（`存活戳已陈旧，但没有可停的事件循环把手` /
`兜底重建请求未能执行`），0.25 秒后连接其实正常 `Authenticated`。
危害是**每次冷启动都误报**，稀释真故障时同样文案的注意力。修法方向：看门狗启动时先等
第一次成功盖戳，或忽略早于本进程启动时刻的存活戳。
