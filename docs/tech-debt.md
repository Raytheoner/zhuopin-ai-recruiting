...
## TD-41 · 0 字节附件全链路无症状，核对器与它完美自洽

**登记时间**：2026-09-10（`[Mac]0909AZ` 从 `0909AP` 泳道报告转交；Shao Peishen 答 `3a`）
**优先级裁定**：🟡 **随第 7 章通道层一起做**（他答 `3a`）。8.6 灰度当天的人工兜底＝
`find data/liaison/archive -type f -size 0`，已写进 `docs/findings/2026-09-09-入站带附件链路预演.md`。
**触发条件**：第 7 章通道层开工时一并处理；⛔ 在此之前不单独占泳道。

**缺口**：一份 **0 字节**附件会完整走通——落盘成功 → 台账写入 → 待办生成 →
`consistency.py` 核对器报**空清单**。核对器核的是「台账说的长度/摘要与盘上是否一致」，
而 `byte_length=0` 与空串的 SHA-256（`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`）
**完美自洽**，所以全链路零症状。

**为什么是缺口**：`b""` 有两个来源，系统分不开——① 用户真发了个空文件（罕见但合法，⛔ 不该拒收）；
② 下载/解密环节静默失败返回空。第 ② 种是**材料丢失**，却和第 ① 种长得一模一样。

⛔ **不要改成「归档层拒收 0 字节」**——那会误伤第 ① 种。

**现状钉子**：`test_a_zero_byte_attachment_is_archived_and_enqueued_with_no_symptom`（`7cafbfc`）
已把当前行为钉住，⛔ 它不是回归岗，是"改动此处会先撞到它"的提醒。

**还债动作（待第 7 章）**：在**下载侧**（而非归档侧）区分两个来源——通道层拿到 0 字节响应时
...
## TD-48 · 无头拆件会话在占位 prompt 下自行改写并提交 `docs/session接力.md`
