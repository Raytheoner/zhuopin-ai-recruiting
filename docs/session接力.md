...
- 2026-09-17 拆件会话（真实起活，非 `--force` 验收）再探同一 pending：`人事部#1`/`MSGID0001`（`at` 2026-09-10T10:00:00+08:00）。复核结果与 R-3、TD-48 一致：`README-跟进信清单.md` 该行仍 `✅ 已推送 2026-09-09`（非第九态），`archived_path`（`data/liaison/archive/threadA/20260910/MSGID0001__正文.txt`）仍不存在。命中章程 §四第 3 条，未新建 R 行（信息已由 R-3 承载，避免重复 `07537fe`/`748a323` 的噪声模式）。【谁做：Shao Peishen】【状态：待人】【判据：该行台账手工转第九态并补上归档件，或确认为陈旧信号后 `unpack-signal --clear --before 2026-09-10T10:00:01+08:00`】【不做会怎样：pending 项保留，下次拆件仍会再探到同一条，不会丢】
