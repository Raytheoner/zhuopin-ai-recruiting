...
| **R-9** | `.51` 装 VC++ 2015–2022 x64 Redistributable（现 v14.27 过旧，torch 2.14 `c10.dll` WinError 1114）后重跑 `smoke_m2_deps` 与 `bench_bge_m3`，填 `docs/m2-model-comparison.md`「BGE-M3 本地 CPU 召回」的 `.51` 耗时 | 另起 session（改 `.51` 系统组件，环境操作·代理人未设 ⇒ 等本人放行） | ⏸ **待派发**（`0917AX` 登记） | `.51` 隔离 venv `C:\apps\m2-smoke` 里 `import torch` 成功、探针 torch/FlagEmbedding 行 ✅ | D7「BGE-M3 在 `.51` 本地 CPU」在现网跑不起来，M2 U3 召回上线被卡 |
| **R-10** | `smoke_m2_deps.py` 在 GBK 控制台 `print` 崩（`UnicodeEncodeError`）→ 脚本自带 `sys.stdout.reconfigure(encoding="utf-8")` | M2 U1 泳道顺带 | ⏸ 待登记 TD | `.51` 不设 `PYTHONUTF8` 裸跑 `EXIT=0` | 每次在 Windows 跑探针都得手动加环境变量 |
...
## 2026-09-17 拆件会话（自动，第二轮，msgid `c61d35...`）
