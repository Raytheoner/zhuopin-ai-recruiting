...
| **G-4** | 在 Terminal 跑 `install_launchd.py`（不带 `--dry-run`） | **Shao Peishen**（脚本 docstring 明写「Claude ⛔ 不代跑」） | ⏸ **等 Shao Peishen 拍板装机**（2026-09-09 `[Mac]0909AJ` 起）。**解锁条件三项已全部满足**：TD-39 已还（`62d3594`＋`9a35912`）、有自动化测试覆盖（`test_session_client_reconnect.py`）、真实断网复验通过（`0909AJ`，断网 245 秒，④⑤⑥ 全绿）。⛔ **装机本身仍是他本人在 Terminal 跑的不可代项**，`0909AJ` 未装也不许代装。⚠️ 装机前请他知悉一条**已知未修**项：冷启动瞬间看门狗会误报两条 ERROR（读到上轮残留存活戳），无害但会稀释真故障告警的注意力 | ~~🔴 现在装比不装更危险~~ —— **该理由 2026-09-09 已消解**：`0909AG` 的「断线后永不恢复、永不告警」已由 `0909AJ` 实测推翻（复网 23.5 秒自愈、窗口闭合并发出恢复告警）。现在剩的只是他本人拍板与动手。`--dry-run` 已于 09-09 验过，渲染无误、四条路径正确、无凭据取值 | 见左栏——**不是**「装了没用」，是「装了会掩盖故障」 |
