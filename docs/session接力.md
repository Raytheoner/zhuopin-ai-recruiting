...
| **AS-1b** | 主工作区两份**未跟踪**的旧稿 opener 处置：`docs/openers/0910A-解锁追平与TD42真实验证.md`（`0909AS` 出，号池 0910A 行仍写「正文未写」）＋ `docs/openers/0910B-SDK消息事件接线.md`（`0909AS` 出的旧稿，与已跟踪正本 `0910B-SDK-message事件接线.md` **同题不同名**） | **`[Mac]0910A` 接手时自决**（2026-09-10 13:3x Shao Peishen 答 `1a`，`[Mac]0909AV` 落档） | ⏸ 待 `0910A` 开工 | `0910A` 正文：要么提交它并把号池 0910A 行「正文未写」改掉，要么删掉重写；`0910B` 旧稿：**删**（正本已在 `main`）。`git status` 里这两份不再出现 | 两份同题 opener 并存，下一个人贴错一份就白跑一轮（`0910C` 撞号已演过一次） |
