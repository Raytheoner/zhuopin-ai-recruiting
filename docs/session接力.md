...
| `.51` 代码 | 🔴 **停在 08-19**（`app\config.py` 34 行、`app\audit\` 整包缺失）。08-27 起的 audit 功能从未部署过去——这是 §5-3 四步验证两轮都跑不通的真因（`0903A` 09-03 查明，见 `docs/audit-and-outbound-ops.md` §五第 3 项） |
...
### ② `[Mac]0903D`：`.51` 阶段 D 日常发版 ＋ 发版后补跑 §5-3 四步 —— **已授权，待跑**
