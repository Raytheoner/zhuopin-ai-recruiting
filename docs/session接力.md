...
| **R-7** | 路线图四项裁决（09-17 答 `1a，2a，3a，4a`）：G3 泳道结果私信本人／G4 Mac 侧调度器／G7 撤包 `lane-launch-armed-scan`／M2 需求树由 Cowork 起草 | Cowork 编排；M2 需 Shao Peishen 逐题答 | ⏸ G3→G4 与撤包的泳道块等本批 `0917T`–`X` 收敛后发车（发车器不允许两批并发）；M2 第 1 轮 10 题已答（全推荐）⇒ `docs/roadmap/M2-intent.md`，待 propose 泳道 | 三条泳道 OK 合入；M2 十题答完 → intent → propose | 自动化缺口 G3/G4 长期挂着；M2 迟迟不立包 |
