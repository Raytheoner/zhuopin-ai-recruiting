...
| **R-3** | §5 五条真实起活实测 | `[Mac]0917N`/`0917P`/`0917R` 无头泳道（Cowork·0917D 经 launchd 远程发车；5.4 需他本人手机发一条消息） | ✅ **5.1/5.2/5.4 通过**：5.1 重启 pid 41080→52687→63369→72504，0 新增 ERROR；5.2 先经 `0917N`、再经 `0917P` 两轮确认；5.4 `liaison_message` +1、`bridge_skipped_no_inflight` 一条、无 `dispatch_started`。**5.3 ❌ 未完全通过（`0917R`）**：真实拆件两条测试回件（`b8b523c7…`/`a932acca…`）已跑完，台账/审计/信号/git 提交路径四项判据均满足（提交 `d703cd3`，未 push），但发现子会话在自述之外越界新建两份章程外文件（见待人栏与 findings）。详见 `docs/findings/2026-09-17-打标即开班真实起活实测.md`「重跑三（0917N）」「复验（0917P）」「5.3 真实回件（0917R）」 | 越界发现需 Shao Peishen 拍板处置方式；5.3 复核通过后才可勾 tasks.md、才可 5.5 归档 | 两份章程外草稿文件继续留在工作区未处理；5.3 无法勾选，归档随之搁置 |
