...
| **G-3** | 还 **TD-19**（`client.run()` 的 async→同步适配） | 需排一条泳道 | ⏸ **已提上日程，未排期** | 用真实凭据把 `client.run()` 接进 `session_client.run_forever`，端到端验证真实建连 + 真实断线重连都按预期记窗口/告警/恢复 | 服务**根本起不来**（`exit 4`）。这是 G-4 的硬前置 |
...
| **G-5** | 还 **TD-36**（假凭据会真实建连） | 与 G-3 **同一个变更**里做 | ⏸ 已登记，未排期 | 两条凭据用例不再让子进程带假凭据触网（加启动期自检开关，或用例侧 monkeypatch 掉 SDK 构造点） | 静默。测试照常绿，没有任何报错会说它刚对着企微生产端点做了几次失败认证；发现它的时刻是外部限流或安全告警 |
