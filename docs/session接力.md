...
| **R-3b** | TD-47：拆件子进程未登录 | `[Mac]0917K` 已修（`8ce57b3`，补 `USER`，已验证）→ `[Mac]0917M` 尝试 cherry-pick 合入，**再次止步** | ⏸ `[Mac]0917M`：K 分支顶部已变成 `fe64bc1`（多出「第三轮验收起活」R-3e 记录，未改 `8ce57b3` 内容本身，但触发 opener 字面预案「不是 8ce57b3 ⇒ 停」），未 cherry-pick、未合并、未清理 K worktree；TD-48（占位 prompt 子会话自发提交接力噪声）已独立登记，与合并结果无关；详见 `docs/findings/2026-09-17-打标即开班真实起活实测.md`「重跑二（0917M）」 | TD-47 在 main 上划掉、5.2 日志无 `Not logged in`、5.4 通过 | 5.2/5.3/5.5/归档持续卡死；每次重派都会在同一检查点原地打转，需人判断 `fe64bc1`（worktree 下 `data/liaison/` 运行时状态结构性缺失）是否影响可合并性，或直接授权只取 `8ce57b3` |
| **R-4** | 汤丽萍解锁 | — | ✅ **已解锁**（§0 的 0.3 已勾＝真实入站已落库一条） | — | — |
| **R-5** | `hr-wecom-aibot-liaison` 8.6–8.9 灰度四条 | Shao Peishen 本人 | ⏸ 等本包 §5 | 四条全勾并归档 | 值守通道停在「代码齐、没真用过」 |
| **R-6** | 拆件会话探到信号但按章程 §四止步：`unpack-signal.json` 里 pending 一条 `人事部#1`／`MSGID0001`（`at` 时刻 `2026-09-10T10:00:00+08:00`），但 `docs/跟进信/README-跟进信清单.md` 该行当前状态是 `✅ 已推送 2026-09-09`，**不是**第九态 `📨 回件已到，待拆件`；且信号里的 `archived_path`（`data/liaison/archive/threadA/20260910/MSGID0001__正文.txt`）在仓库中不存在。命中章程 §四第 3 条「台账行已不处于第九态」，本轮未拆件、未清信号 | Shao Peishen | 待人 | 人工核实：该信号是否为 AT-1b 测试期遗留的陈旧信号（`R-4` 显示真实入站早已通过另一条落库解锁，本条大概率是先于 P2 章程建成前的旧探针数据）——若是 ⇒ 清除信号（`unpack-signal --clear --before 2026-09-10T10:00:01+08:00`）；若确有未处理的真实回件 ⇒ 先把该行台账手工转成第九态，再由下一次拆件会话重跑 | 该 pending 项保留，下一次拆件探测（打标即开班或手动 `/liaison-unpack`）仍会命中同一条、同样在此止步，不会丢也无法自动推进 |
