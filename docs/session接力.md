...
且派的是**引用式 opener**（正文指向 `docs/openers/0910A-解锁追平与TD42真实验证.md`），
...
| **AS-3** | **给号池加机器强制**：写一个双向对账脚本（扫 `docs/openers/<MMDDX>-*.md` 与 `OP-0820-全量编排.md` 号池表，缺任一侧即报非 0），接进本地校验入口 | **另起 session**（`[Mac]0910C` 只登记、⛔ 不动手——本项要另起 session，按 CLAUDE.md §「需你定夺」路由规则不给选项） | ⏸ **待派发**（2026-09-10 Shao Peishen 答 `2a`：立此条） | 脚本存在且可跑；**判据＝「每条号池行要么有 opener 正文文件，要么在『去向』列带一个显式豁免标记与理由」**，⛔ 不许把「无正文」一律判成缺口；对当前仓库跑一遍应**零缺口**（`0910B` 带豁免标记 ⇒ 放行，`0910A`／`0910C` 有正文 ⇒ 放行）；接进 CI／本地 hook 后，派号漏落档当场非 0。⚠️ **本判据是 2026-09-10 改过的**：原写「能报出『`0910B` 有台账行但无正文』」，被同日 `1a` 的结论证伪——`0910B` 是已执行完的号，按原判据它会被脚本**永久报成假阳性**，而一个天天报假阳性的门禁三天之内就会被所有人忽略，等于没有 | AS-2 的判据已于 2026-09-10 写进真源（`8e4674e`）却**当天就被绕过一次**（`0910C`）⇒ 证明纯规则约束不起作用。按 08-27（撞号 5 次）、09-10（断链 2 次）的频率，下次发作大概率在一周内，且**全程不报错**——接手 session 只会扑空、零改动停下，白耗一整轮 |
| **AS-4** | 查明并确认 `[Mac]0910B` 是哪个 session、干了什么（派号当时只补了号池行，主题栏是空的，核队列的人分不清它是漏跑还是取消） | **Shao Peishen 或 `0910B` 母 session** | ✅ **已销（2026-09-10 Shao Peishen 答 `1a` 确认）**：主题＝「`0910A` 扑空后的补登与判据落档，并派出 `0910C`」，号池已去掉「待确认」标记。⚖️ **正文文件不补**——opener 正文的用途是给接手 session 在开工那一刻读，本号早已执行完（产出即 `beefc6f`／`8e4674e`），事后补写没人会读的正文收益为零；号池行已挂显式豁免标记。⚠️ 不违反 AS-2（它约束的是**派号那一刻**，不是历史号） | `docs/openers/` 下存在 `0910B` 的正文文件，且号池「主题」列换成真实主题 | 09-10 的派号链断了**两层**。只补 `0910C` 而留着 `0910B` 空着，下次核队列看见 `0910B` 仍然分不清是漏跑还是取消——而这两者的处置完全相反 |
...
| **H-5** | 订正 `docs/跟进信/README-跟进信清单.md` 里「`bf63e7a` 入库时把本行写成终态」那句失真（答 `5a`） | **另起 session**（`docs/跟进信/` 是他的触碰区，`0909AZ` ⛔ 未碰） | ⏸ **待派发** | 那句改为「来源不明的未提交工作区状态」，台账与版本库口径归一 | 下次按这句去查 `bf63e7a` 会扑空；且「台账状态是机器判据」那条教训的**依据本身是错的** |

⚠️ **H-1 / H-4 / H-5 三条尚无 opener**，派发时从 `0909AR` 起连号取，**取号当场登记进 `OP-0820` 号池台账**。
