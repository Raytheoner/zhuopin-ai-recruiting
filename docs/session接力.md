...
- ⏸ **`0909AH`（TD-39 重连链修复）在跑**，未落地。它是**唯一阻断 8.6** 的一条；G-4 装 launchd 继续锁，解锁条件＝TD-39 还上 + 自动化测试 + 真实断网复验通过
...
| **G-4** | 在 Terminal 跑 `install_launchd.py`（不带 `--dry-run`） | **Shao Peishen**（脚本 docstring 明写「Claude ⛔ 不代跑」） | ⛔ **继续锁**（2026-09-09 他答 `3 继续锁`）。解锁条件已改为：**TD-39 还上、有自动化测试覆盖，且真实断网复验通过**（⛔ 不再是「G-8/G-9 都通过」——G-9 已通过，但结论是缺陷） | 🔴 **现在装比不装更危险**：`0909AG` 实证服务断线后**永不恢复、永不告警**，把它设成开机自启＝让一个「断了就再也回不来、还不吭声」的服务无人值守地跑。`--dry-run` 已于 09-09 验过，渲染无误、四条路径正确、无凭据取值 | 见左栏——**不是**「装了没用」，是「装了会掩盖故障」 |
