...
| **AS-2** | **引用式 opener 的被引文件必须与派号同一次落档并提交**——`docs/openers/<MMDDX>-<主题短名>.md` ＋ `OP-0820` 号池台账两处同时写 | 所有派发方（CC／Cowork 两端同等适用） | ⏸ **待确认是否写进真源**（`.claude/skills/kickoff/SKILL.md`「引用式 Opener」段），本次只在此登记 | 派号的那一条 commit 里同时出现 opener 正文文件与号池台账行；接手 session 不再出现「引用式 opener 指向不存在的文件」 | 引用式 opener 退化成**死链**：接手方既读不到正文、又在台账里查不到号 ⇒ 下轮必被重派或再扑空，且**全程不报错**（08-27 撞号 5 次是同一根因的第一次发作，本次是第二次） |
