...
| **G-4** | 在 Terminal 跑 `install_launchd.py`（不带 `--dry-run`） | **Shao Peishen**（脚本 docstring 明写「Claude ⛔ 不代跑」） | 🟢 **已解锁、已授权装机 —— ⏸ 只等他本人在 Terminal 跑那一条命令**（Shao Peishen 2026-09-09 答 `1a`）。解锁条件三项全部满足：TD-39 已还（`62d3594`＋`9a35912`）、自动化测试覆盖（`test_session_client_reconnect.py`）、真实断网复验通过（`0909AJ`，断网 245 秒，④⑤⑥ 全绿）。`0909AJ` 已代跑 `--dry-run` 复核：四条路径正确、解释器存在、plist 无凭据取值、`~/Library/LaunchAgents/` 下**尚未装过**（首次安装，非重装）。⛔ **命令本身仍不可代跑**（脚本 docstring 明写「Claude ⛔ 不代跑」，属安全配置变更）。⚠️ 装完看日志时：开头那两条 ERROR 是**已知噪声**（冷启动误报，他答 `3b` 明确不修），⛔ 不要按真故障查 | ~~🔴 现在装比不装更危险~~ —— **该理由 2026-09-09 已消解**：`0909AG` 的「断线后永不恢复、永不告警」已由 `0909AJ` 实测推翻（复网 23.5 秒自愈、窗口闭合并发出恢复告警）。他已答 `1a` 授权装机，**现在只差他本人跑那一条命令**。`--dry-run` 已于 09-09 验过，渲染无误、四条路径正确、无凭据取值 | 见左栏——**不是**「装了没用」，是「装了会掩盖故障」 |
...
| **G-12** | **N-0 兜底层（存活戳看门狗）端到端覆盖** | 泳道 `[Mac]0909AP`，opener 已就位：`docs/openers/0909AP-N0看门狗端到端覆盖.md`（worktree ☑） | 🆕 **已出 opener，⏸ 待派发**（Shao Peishen 2026-09-09 答 `2a` 授权）。触碰区 `test_session_client_reconnect.py`，与第十六批（AK–AO）**不重叠**，可并行 | `0909AJ` 实测证明 TD-39 两层修法里**只有改法 1 被真实验证**，看门狗那层至今只有单测。🔴 ⛔ **那条不要再去断 Wi-Fi**——改法 1 生效后 `error` 事件每 ≤30 秒刷新一次存活戳，**永远达不到 180 秒阈值**，普通断网**按设计**就触发不了看门狗。必须在测试里构造「SDK 事件完全静默但 loop 还活着」的形态（＝`0909AG` 撞到的原始故障）。三条 E-1/E-2/E-3 各须**先红后绿** | 🔴 兜底层没被端到端验过 ＝ **SDK 里任何别的「task 死了但 loop 还活着」的形态都会重演 `0909AG`**，而症状同样是「服务看起来在跑、其实早断了」 |
...
**G-4 已解锁并经他答 `1a` 授权，只差他本人在 Terminal 跑一次**（不可代跑，`--dry-run` 已由 `0909AJ` 代验过）。
G-12 待派发（可与第十六批并行）；G-6 等他一个「发」。
⇒ **球在他手上的只有 G-4 与 G-6 两件**，其余都是可派发的泳道活。

**🔢 号池登记（2026-09-09）**：`0909AK`–`AO` ＋ `AZ` 由第十六批编排占用（见 `OP-0820` 未提交段），
`0909AP` 由本条（G-12）占用。⚠️ **下一个可用号是 `0909AQ`**。
⛔ **`0909AP` 尚未落进 `docs/openers/OP-0820-全量编排.md` 的号池台账**——那份文件此刻带着
第十六批编排 session 的 **161 行未提交改动**，`git add` 它会把别人的活一并提交（违反并行协议 #1）。
🔴 **下一个动 `OP-0820` 的 session 请顺手补登 `0909AP` 这一行**，⛔ 不要因为台账里没有就重派该号。
