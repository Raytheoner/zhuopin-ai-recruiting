...
| **P-3** | 🔴 **TD-42 真实验证** ＝ `0910C`（SDK `message` 接线）的硬前置 | **Shao Peishen 本人**（不可代：要 `.env` ＋ 真实建连） | ⏸ **未做**。判据见 `docs/tech-debt.md` TD-42 末段「⏸ 真实验证留步」那一节 | ① `cat data/liaison/liveness.json` 看 `last_event_at` 是否每 ~30 秒推进；② 断网/静默 ≥ 13 分钟，确认日志出现「看门狗终止进程」、进程被 launchd 拉起、`data/liaison/watchdog.json` 计数 +1。做完把 TD-42 那节改成「✅ 已验证」并附证据 | `0910C` 开工即在第二道闸门停车（opener 已写死）。⇒ 8.6 灰度继续卡住，且下一次 `liaison_message` 为 0 时仍分不清是假死还是没接线 |
| **P-4** | `0910C` 发车（SDK `message` 事件接线） | Shao Peishen 贴引用块 → CC worktree session | ✅ **opener 正文已就绪**：`docs/openers/0910C-SDK消息事件接线.md`（`[Mac]0910B` 出，2026-09-10）。⏸ 等 P-3。两项裁决已写进正文：`1a` 守卫测试翻转、`2a` TD-42 为硬前置 | 接线合回 main、四条测试先红后绿、`liaison_message` 在假 SDK 端到端下 +1 | `handle_inbound_message` 继续零生产调用方——专员发的每一条都静默落空，而所有仪表显示绿灯 |
