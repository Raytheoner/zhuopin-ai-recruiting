...
| **M0′ 回件自动化** | `liaison-reply-bridge-and-patrol` 回件桥＋打标即开班＋章程＋口径台账 | 32/33；5.1–5.4 ✅ | 5.5 落档＋归档（判据已满足，归档时限要求当场） | 泳道（`0917U`） |
...
| **M2 简历解析与评分排序**（护城河） | 解析管线、200 份私有评测集、硬门槛引擎、BGE-M3 召回＋rubric 精排＋evidence span、HR 工作台、合规验收 #1 | **未立包** | 三个硬前置：① 需求 grill → openspec propose；② **200 份评测集要 HR 出人标注**；③ **PIA／合规验收 #1 阻塞真实简历入库** | ① Cowork 起草需求树＋你答；② ③ 跟进信线 |
...
| ⑤ 发车 | ✅ 请求文件路（09-09 修好 plist 后稳定） | `lane-launch-armed-scan` 那套「定时扫＋武装令牌」已搁置，请求文件路已够用 |
...
| G7 | `lane-launch-armed-scan` 0/35 搁置包悬挂 | 进度表长期挂一个不做的包 | 撤包或正式延后 | ⏸ 待你定（文末 3） |

## 五、本轮（2026-09-17 午）已发车

| 泳道 | 编号 | 做什么 | 环境 |
|---|---|---|---|
| 收尾落档（串行） | `0917T` → `0917U` → `0917V` | T：提交本文与 Cowork 未提交文档；U：回件包 5.5 落档＋`openspec-archive-change`；V：值守包 8.6/8.7 按实测证据核证勾选、8.8 观察窗开窗登记（到期 09-24） | 主工作区 |
...
1. G3 泳道结果通知：私信你本人 vs 不做
2. G4 调度器落点：先做 Mac 侧（值守/跟进信提醒）vs 先做 `.51` 侧（M1 挂起提醒）
3. G7 `lane-launch-armed-scan`：撤包 vs 延后
4. M2 启动方式：本 Cowork 起草 M2 需求树（grill 题）你逐题答 vs 另开专场
