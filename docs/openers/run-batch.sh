#!/usr/bin/env bash
# ⚰️ 已被 run-lanes.sh 取代（2026-08-26），保留供追溯，不要再用这份。
#
# 为什么换掉：本脚本只有"波次"（全局同步点）且**只看退出码**。而 claude -p 退出码 0
# 只说明进程正常结束，**不说明任务完成**——2026-08-20 单元 B 那次就是日志看着正常、
# main 上什么都没有。run-lanes.sh 补上了哨兵双指标（OPENER_DONE/OPENER_PARTIAL）、
# 泳道（各自串行、互不等待）、无头引导头、错峰启动、失败只停本泳道。
#
# → 用 docs/openers/run-lanes.sh
#
# ---------------------------------------------------------------------------
# 以下为原内容
# OP-0820 批量执行器
