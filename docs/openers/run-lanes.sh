...
# ---------------------------------------------------------------------------
# 🔴 自拷贝执行（2026-09-08 加，第十二批实证）
#
# 由来：bash 是**按字节偏移边跑边读**源文件的，不是一次性载入。第十二批把
# 「改 run-lanes.sh」的 opener 放进它自己驱动的泳道，那条 session 在编排器
# 运行期间重写了 run-lanes.sh —— 运行中的 bash 拿着旧偏移续读**新文件**，
# 从半截语句处接着解析，编排器自我损坏，六条 opener 被派了两遍
# （见 lanes-20260908-171737-第十二批看护报告.md §二、§九）。
#
# 根治办法是让运行实例与仓库里的文件脱钩：开跑第一件事把自己拷到临时目录，
# 再 exec 那个副本。此后仓库里的 run-lanes.sh 怎么改都碰不到正在跑的这一份。
# 放在 set -uo pipefail 之后、其余一切之前，是为了把「按旧文件读」的窗口压到最小。
#
# ⚠️ REPO / PLAN 仍是仓库绝对路径常量，⛔ 不要改成相对 $0 —— 副本在 /tmp 下，
#    相对 $0 会把工作目录指到临时目录，整条链路当场失准。
# 参数 --only / --chain / --dry-run 等由 "$@" 原样透传；dry-run 与实跑同走这条路。
# 副本不删：正跑着的脚本删自己的源文件正是上面要躲的那个坑，且 /tmp 由系统回收。
# ---------------------------------------------------------------------------
if [[ -z "${RUN_LANES_COPY:-}" ]]; then
  _rl_dir="$(mktemp -d "${TMPDIR:-/tmp}/run-lanes-XXXXXX")" || {
    echo "✗ 无法创建自拷贝临时目录" >&2; exit 1; }
  _rl_copy="$_rl_dir/run-lanes.sh"
  cp "$0" "$_rl_copy" || {
    echo "✗ 无法把 $0 拷到 $_rl_copy（cwd=$PWD）" >&2; exit 1; }
  echo "▶ 自拷贝执行：$_rl_copy"
  echo "  （运行实例已与仓库文件脱钩，本次运行期间改 docs/openers/run-lanes.sh 不影响它）"
  RUN_LANES_COPY=1 exec bash "$_rl_copy" "$@"
fi
...
[[ -s "$MANIFEST" ]] || {
  # 自检行也要在这条零条目路径上打印（2026-09-08 加）。下面那段「条目总数自检」
  # 在 manifest 为空时根本走不到（这里已经 exit 12 了），于是队列跑空的 dry-run
  # 只看得到一句"没解析到"，看不到 Σ/N/M 三个数——而"队列空了"与"标注写了但解析不出"
  # 是两件处置完全相反的事，光靠那句话分不出来。
  if [[ "$PLAN_LANE_M" -eq 0 ]]; then
    echo "条目总数自检：Σ=N=M=0"
    echo "✗ 编排文件里当前没有任何待执行条目（'> 泳道：' 标注数为 0）。"
    echo "  队列已跑空是正常状态；要发车先在编排文件里给 opener 代码块**前一行**补：  > 泳道：<泳道名>"
  else
    echo "条目总数自检：Σ=0  N=0  M=${PLAN_LANE_M}"
    echo "✗ 有 ${PLAN_LANE_M} 处 '> 泳道：' 标注，却一条 opener 都没解析出来。"
    echo "  多半是标注下面代码块的首行不合抬头格式（现行格式 [Mac]MMDDX-<主题短名>）。"
    echo "  自查：grep -n -A3 '^>[[:space:]]*泳道：' \"$PLAN\""
  fi
  exit 12
}
...
if [[ -n "$ONLY" ]]; then
  ...
  mv "$tmp" "$MANIFEST"
  [[ -s "$MANIFEST" ]] || {
    echo "条目总数自检：Σ=0  N=0（--only 过滤后为空，M=${PLAN_LANE_M} 不参与判据）"
    echo "✗ --only 过滤后为空：$ONLY 里的编号在 manifest 里一个都没匹配上"
    exit 12
  }
fi
