...
判据②的由来（2026-08-27，企业AI转型侧正本 §〇.0 补充三）：
  ...
  它的 session 名是**摘要生成**的——2026-08-27 实测：开场词首行 `HR业务线-接力0827B`
  → 侧边栏 `HR业务线接力`，连字符与编号一并被改写掉，既非原样也非截断。
  ⇒ 此前那句「取自开场词首行、语义在前编号在后」**已被推翻**，⛔ 不要再改首行格式硬试。
  故判据②只对写着「执行环境: CC」的块生效。

  ⚠️ 本判据扫的是 **Claude 的输出**，扫不到躺在文件里的存量 opener——
  `docs/openers/*.md` 那一侧由 `run-lanes.sh` 的开跑前自检守（2026-08-27 加）。
  这个缺口是实证出来的：08-27 定下本规则当天，编排文件里一条 opener 都没回填，
  而两边都没有任何症状。
